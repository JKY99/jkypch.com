...
<p style="margin:0 0 0.3rem;font-size:0.85rem"><a href="#s7" style="color:#6a9aef;text-decoration:none">4. NLDD 실전 가이드</a> <button id="nldd-toggle-all" style="margin-left:0.5rem;font-size:0.7rem;padding:0.15rem 0.5rem;background:#1a1a2e;border:1px solid #2a2a3a;border-radius:4px;color:#6a9aef;cursor:pointer">모두 펼치기</button></p>
...
<h2 id="s7"><a href="#toc">4. NLDD 실전 가이드</a></h2>
...
여러 기능을 한 명세에 몰아넣으면 LLM의 출력 품질이 떨어진다. 명세 하나에 목적 하나를 유지한다.
...
회원가입, 마이페이지, 소셜 로그인은 각각 별도 명세로 분리한다.
...
구현 방법부터 지시하면 LLM이 더 나은 대안을 제시할 기회를 잃는다. 목적을 먼저 말하고, 제약이 있을 때만 구현 방식을 지정한다.
...
모호한 제약은 모호한 코드를 만든다. 숫자, 조건, 기준을 명시한다.
...
LLM은 프로젝트를 모른다. 기술 스택, 디렉토리 구조, 기존 패턴을 알려줘야 일관된 코드가 나온다.
...
[맥락]
  ...
  - 기존 패턴 참고: PostController.java의 getPost()
  - MongoDB 사용, MongoRepository 상속
...
정상 동작만 기술하면 예외 처리가 빠진다. "안 되는 경우"를 반드시 포함한다.
...
NLDD는 일회성 프롬프트가 아니라 반복적 정교화 과정이다.
...
각 라운드에서 검증 → 발견 → 보완을 반복한다. 자연어의 정밀도가 매 라운드마다 올라간다.
